...
st.title("대화 이력 조회")  # Streamlit 화면의 가장 큰 제목을 표시합니다.
...
items = response.json().get("items", [])  # API 응답 JSON에서 목록 데이터를 꺼내 화면 표시용 변수에 저장합니다.

for item in items:  # 목록이나 반복 가능한 데이터를 하나씩 꺼내 같은 작업을 반복합니다.
