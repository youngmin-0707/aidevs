...
st.title("캐시 사용자 대시보드 실습")  # Streamlit 화면의 가장 큰 제목을 표시합니다.

@st.cache_data(ttl=30)  # 이 줄은 예제의 핵심 동작을 단계별로 보여주기 위한 코드입니다.
def load_items(token):  # 반복해서 사용할 로직을 함수로 묶어 이름을 붙입니다.
...
items = load_items(TOKEN)  # API 응답 JSON에서 목록 데이터를 꺼내 화면 표시용 변수에 저장합니다.
