...
students = [  # 딕셔너리 데이터를 행과 열을 가진 DataFrame 표 구조로 변환합니다.
...
students_sum = df["score"].sum()  # 계산 결과나 입력값을 이후 코드에서 다시 쓰기 위해 변수에 저장합니다.



col_students, col_average, col_sum = st.columns(3)  # 메인 화면을 여러 열로 나누어 대시보드 요소를 배치합니다.
...
with col_sum:  # 파일, 화면 영역, 로딩 상태처럼 시작과 종료가 있는 작업 범위를 만듭니다.
    st.metric("평균합계", f"{students_sum}점")  # 핵심 숫자나 상태값을 대시보드 지표 형태로 표시합니다.
